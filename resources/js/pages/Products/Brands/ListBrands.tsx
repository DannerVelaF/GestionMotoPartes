...
export default function ListBrands({ brands, filters }: Props) {
    // --- ESTADOS ---
    ...
    // Paginación Editable
    const [perPage, setPerPage] = useState<string | number>(brands.per_page);
    ...
    const handlePerPageSubmit = () => {
        ...
        if (newValue > brands.total) newValue = brands.total;
        else if (newValue < 1) newValue = 20;

        setPerPage(newValue);

        if (newValue !== brands.per_page) {
    ...
    const handleRowClick = (brand: Brand) => {
        router.visit(brandsRoute.show({ brand: brand.id_brand }).url);
    };
    ...
    const executeBulkDelete = () => {
        ...
        const selectedIds = selectedIndices.map(
            (index) => brands.data[index].id_brand,
        );

        router.delete(brandsRoute.bulkDestroy().url, {
    ...
    const hidePaginationControls =
        brands.total <= brands.per_page || brands.total === 0;
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            ...
                            <strong>{selectedCount}</strong> marcas
                            seleccionadas permanentemente.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        ...
                        <div className="flex w-full animate-in items-center justify-between fade-in slide-in-from-bottom-1">
                            <div className="flex items-center gap-4">
                                <Button
                                    ...
                                    onClick={() =>
                                        router.visit(brandsRoute.create().url)
                                    }
                                >
